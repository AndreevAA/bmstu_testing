...
def test_correct_add_item():
    ...
    username = "larin"
    user_repo.register_user(username, "Vladimir Larin", "12345678", None)
...
def test_correct_update_item():
    ...
    user_repo = UserRepository(DB_NAME, False)
    username = "andreev"
    user_repo.register_user(username, "Alexander Andreev", "12345678", None)
    ...
    assert item_repo.get_item_by_id(responsItemAdd["item_id"]) == {'clothes_id': 1, 'clothes_name': 'new_name', 'type': 't-shirt', 'image_id': None, 'owner_login': 'andreev', 'image_url': None}
