...
def test_who_invites_me_valid(repo, service):
    repo_return = [
        {
            'login_that_invites': 'andreev',
            'wardrobe_name': 'gucci flip flops',
            'image_url': None
        },
    ]
    PyMock.setup(repo.get_invites_by_login('larin')).returns(repo_return)

    response, code = service.who_invites_me('larin')

    assert code == 200
    assert response == '[{"login_that_invites": "andreev", "wardrobe_name": "gucci flip flops", "image_url": null}]'
