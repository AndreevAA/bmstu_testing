...
def test_correct_invite():
    ...
    wardrobe_builder = WardrobeBuilder().with_name("gucci flip flops").with_creator_login('larin').with_description('A')

    _, id = wardrobe_repo.create_wardrobe(*wardrobe_builder.build_create_data())
    result, error = invite_repo.inviteUser('larin', 'andreev', id)
...
def test_no_such_wardrobe():
    ...
    wardrobe_builder = WardrobeBuilder() \
        .with_name("gucci flip flops").with_creator_login('larin').with_description("No comment")
    ...
    response, error = invite_repo.inviteUser('larin', 'andreev', id + 5)
...
def test_corrent_get_invites():
    ...
    wardrobe_builder = WardrobeBuilder() \
        .with_name("gucci flip flops").with_creator_login('larin').with_description("No comment")
    _, id = wardrobe_repo.create_wardrobe(*wardrobe_builder.build_create_data())
    invite_repo.inviteUser('larin', 'andreev', id)

    expect = [{'invite_id': 1, 'login_that_invites': 'larin', 'wardrobe_name': 'gucci flip flops', 'image_url': None}]
    response = invite_repo.get_invites_by_login('andreev')
...
def test_empty_invites():
    ...
    expect = []
    response = invite_repo.get_invites_by_login('larin')
